...
generate_pm <- function(x1,y1,nodes){
...
generate_pm <- function(p,q,nodes){
  pm <- matrix(NA,nrow=nodes,ncol=nodes)
  pi1 <- 0.6
  pi2 <- 0.4
  n1 <- pi1*nodes
...
#Random dot product graph
nodes_list <- rep(100,5)

N <- 200
x1 <- rbeta(N, 1, 1)
y1 <- rbeta(N, 1, 1)
pm1 <- generate_pm(x1,y1,N)

x2 <- rbeta(N, 1, 2)
y2 <- rbeta(N, 1, 2)
pm2 <- generate_pm(x2,y2,N)

ibg <- select_adjm(pm1,N)
ibg1 <- select_adjm(pm1,N)
ibg2 <- select_adjm(pm2,N)
...
for (i in 1:5){
  ...
  for (k in 1:MT){
    sample1 <- sample(1:N,nodes)
    sample2 <- sample(1:N,nodes)
    sample3 <- sample(1:N,nodes)
    A_1 <- sample_from_graph(A, nodes, sample1)
    A_2 <- sample_from_graph(A1, nodes, sample2)
    A_3 <- sample_from_graph(A2, nodes, sample3)
    
    A11 <- get_embed_adjm(A_1, sel_n, sample_node_index,2)
    A22 <- get_embed_adjm(A_2, sel_n, sample_node_index,2)
    A33 <- get_embed_adjm(A_3, sel_n, sample_node_index,2)
...
for (i in 1:5){
  ...
  for (k in 1:MT){
    ...
    A11 <- get_embed_adjm(A,sel_n, sample_node_index,2)
    A22 <- get_embed_adjm(A1, sel_n, sample_node_index,2)
    A33 <- get_embed_adjm(A2, sel_n, sample_node_index,2)
...
mean_df <- matrix(NA,nrow=2,ncol=length(nodes_list))
var_df <- matrix(NA,nrow=2,ncol=length(nodes_list))
for (i in 1:5){
  mean_df[1,i] = mean(repo_T2[i,])
  mean_df[2,i] = mean(repo_T3[i,])
  var_df[1,i] = var(repo_T2[i,])
  var_df[2,i] = var(repo_T3[i,])
}

write.csv(repo_T2,"C:/Users/86181/Desktop/kernel_embed_T2.csv")
write.csv(repo_T3,"C:/Users/86181/Desktop/kernel_embed_T3.csv")
write.csv(repo_T22,"C:/Users/86181/Desktop/kernel_embed_T22.csv")
write.csv(repo_T31,"C:/Users/86181/Desktop/kernel_embed_T31.csv")
...
for (i in 1:length(nodes_list)){
  ...
  #p1 <- 0.6
  #q1 <- 0.3
  #pm1 <- generate_pm(p1,q1,nodes)
  
  #x2 <- rbeta(nodes, 1, 2)
  #y2 <- rbeta(nodes, 1, 1)
  #p2 <- 0.7
  #q2 <- 0.4
  #pm2 <- generate_pm(p2,q2,nodes)
  gi1 <- rbeta(nodes, 1, 1)
  pm1 <- generate_pm(gi1,nodes)
  
  gi2 <- rbeta(nodes, 1, 2)
  pm2 <- generate_pm(gi2,nodes)
...
write.csv(repo_T1,"C:/Users/86181/Desktop/lpvs_embed_T1.csv")
write.csv(repo_T2,"C:/Users/86181/Desktop/lpvs_embed_T2.csv")
write.csv(repo_T3,"C:/Users/86181/Desktop/lpvs_embed_T3.csv")
write.csv(repo_T11,"C:/Users/86181/Desktop/lpvs_embed_T11.csv")
write.csv(repo_T22,"C:/Users/86181/Desktop/lpvs_embed_T22.csv")
write.csv(repo_T31,"C:/Users/86181/Desktop/lpvs_embed_T31.csv")
